...
| UCloud | https://www.ucloud.cn/site/active/regift.html | pending | Mona 已完成注册/登录，但暂未在后台找到专属邀请链接或 `cps_code`；后续找到后再改为 connected。 |
| 锐成云 | https://www.racent.com/affiliate | available | 官方推广联盟页说明有个人专属推广链接、推广订单跟踪和佣金收入查看；需 Mona 注册后复制专属链接。 |
| 网宝 | https://www.idcpay.com/affiliate.html | available | 官方推广联盟页说明云服务器和独立服务器返佣；需 Mona 注册并确认平台资质、规则和提现方式。 |
...
- UCloud：Mona 已完成注册/登录，但暂未找到推广码。该资源先保持 `pending`；后续如果后台出现 U 大使推广素材、邀请有礼链接或产品页 `cps_code` 参数，再改为 `connected`。
